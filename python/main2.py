...
secretcode_status = config["secretcode_status"]
...
bot = commands.Bot(command_prefix=f"{prefix}{prefix}", intents=intents)
...
async def on_message(message):
    ...
    if message.content == f"{prefix}서버주소" or message.content == f"{prefix}서버 주소" or message.content == f"{prefix}마크":
        ...
        await message.channel.send(f"{prefix}홈페이지 (봇이 대신 입력해드렸어요!! XD)")
    ...
    #elif message.channel.id == channelID_eggGame:
    else:
